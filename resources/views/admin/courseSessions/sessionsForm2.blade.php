@if(is_null(old('sessions')))
    ...
    @include('form.input',['type'=>'hidden','name'=>"start_date",'value'=> $row->start_date])
    @include('form.input',['type'=>'hidden','name'=>"end_date",'value'=> $row->end_date])


    <hr>
            <div class="form-layout-footer mg-t-15">
                <button id="more" class="btn btn-warning bd-2 more-sessions">{{ trans('app.Add more sessions') }}</button>
            </div>
...
@elseIf(old('sessions'))
    ...
    @foreach(old('sessions') as $key => $session)
        ...
        @if($loop->last)
            <hr>
            <div class="form-layout-footer mg-t-15">
                <button id="more" class="btn btn-warning bd-2 more-answers">{{ trans('app.Add more answers') }}</button>
            </div>
        @endif
